...
interface CanvasFractalGridProps {
  ...
  /** Enable spacing pulse */
  enableSpacingPulse?: boolean
  /** Pulse amount as a scale (e.g., 0.3 => +30%) */
  pulseAmount?: number
  /** Pulse interval in ms (e.g., 1000) */
  pulseIntervalMs?: number
  /** Pulse duration in ms (portion of interval) */
...
const DotCanvas: React.FC<{
  ...
  ({
    ...
    const withAlpha = useCallback((color: string, alpha: number) => {
      ...
      const rgba = color.match(/^rgba\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([\d\.]+)\s*\)$/i)
      if (rgba) return `rgba(${rgba[1]}, ${rgba[2]}, ${rgba[3]}, ${a})`
      const rgb = color.match(/^rgb\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$/i)
    ...
    const drawDots = useCallback(
      (ctx: CanvasRenderingContext2D, time: number) => {
        ...
        let spacingScale = 1
        if (enableSpacingPulse) {
          const t = time % pulseIntervalMs
          if (t < pulseDurationMs) {
            const p = t / pulseDurationMs
            const eased = 1 - Math.cos(p * Math.PI)
            spacingScale = 1 + pulseAmount * eased
          }
        }

        const effectiveSpacing = dotSpacing * spacingScale
        ...
        for (let i = 0; i < cols; i += skip) {
          for (let j = 0; j < rows; j += skip) {
            ...
            const seed = (((i * 9301 + j * 49297) % 233280) / 233280)
            const baseColor = dotColors && dotColors.length > 0 ? dotColors[Math.floor(seed * dotColors.length) % dotColors.length] : dotColor
            const glowBase = glowColor || baseColor
            ...
            if (distance < waveRadius) {
              ...
              gradient.addColorStop(0, withAlpha(glowBase, dotOpacity * (1 + waveStrength)))
              gradient.addColorStop(1, withAlpha(glowBase, 0))
              ctx.fillStyle = gradient
            } else {
              ctx.fillStyle = withAlpha(baseColor, dotOpacity)
            }
    ...
    <AnimatePresence>
      ...
      >
        ...
        <DotCanvas
          ...
          mousePos={mousePos}
          dotColors={dotColors}
          enableSpacingPulse={enableSpacingPulse}
